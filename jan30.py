...
async def process_annotations(
    ...
    try:
        # Fetch all referents and annotations using the updated function
    ...
    # Initialize the list of used timestamps
    used_timestamps = []
    # Process each referent to extract annotations
    annotations_data = []
    ...
    try:
        # Proceed only if there are enough annotations for topic modeling
        if len(annotations_data) >= 6:
            ...
            # Prepare data for concurrent summarization
            summarization_tasks = []
            ...
            topic_summaries = [
                summary for summary in all_summaries if summary is not None
            ]
            topic_summaries = sorted(topic_summaries, key=lambda x: x["timestamp"])
            logging.info(
                f"Processing annotations took: {time.time() - start_time_4 :.2f} seconds."
            )
            return topic_model, grouped_annotations, topic_summaries, topic_names
        else:
            raise ValueError("Not enough annotations for topic modeling.")
    except Exception as e:
        logging.info(
            "Falling back to direct annotation use due to error or insufficient data: {}".format(
                e
            )
        )
        # Directly use the annotations without topic modeling
        ...
        annotations_with_timestamps = sorted(
            annotations_with_timestamps, key=lambda x: x["timestamp"]
        )
        return None, annotations_data, annotations_with_timestamps, None
    ...
    async with aiohttp.ClientSession() as session:
        for _ in range(max_redirects):
            async with session.get(url, allow_redirects=False) as response:
                if response.status in (301, 302, 303, 307, 308):  # Redirect status codes
